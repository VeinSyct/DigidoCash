...
_uz = Object.assign(_uz, { user: localStorage.user ? JSON.parse(localStorage.user) : { agreed: {}, opened: {} }, hash: location.href.includes("#") ? urlCharDecode(location.href).split("#").slice(-1).toString() : undefined, o: 0, s: 0, buttons: [], q: [], session: (Math.random() + 1).toString(36).substring(7), days: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], months: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], audio: {}, ireadx: (d) => { return d.db.transaction([d.tx]).objectStore(d.tx).get(d.id) }, ireada: (d) => { return d.db.transaction(d.tx).objectStore(d.tx).openCursor() }, iwrite: (d) => { d.x = d.db.transaction([d.tx], "readwrite").objectStore(d.tx); d.x.delete(d.id); d.x.add({ id: d.id, string: d.s }) }, remove: (d) => { d.db.transaction([d.tx], "readwrite").objectStore(d.tx).delete(d.id) }, udata: [], idbn: "psi-data", idbk: "win-data" }), _pm = Object.assign(_pm, { xncode:(e,c,t,i,u)=>{t=t?t:c.includes('<script type="importmap">')?"importmap":c.includes('<script type="text/js-worker"')?"text/js-worker":c.includes('<script type="module">')?"module":"text/javascript";if(c){if(c.includes('type="text/js-worker" id="'))i=c.split('type="text/js-worker" id="')[1].split('">')[0];c=c.replace('<script type="text/js-worker" id="aiPhysics">',"").replace('<script type="module">',"").replace("<script>","").replace("<\/script>","")}s=document.createElement("script");s.type=t;i&&(s.id=i);if(u)s.setAttribute(u.pro,u.val),s.src=u.src;else try{s.appendChild(document.createTextNode(c))}catch(e){s.text=c}try{e.match(/(head|body)/)?document[e].appendChild(s):document.getElementById(e.replace("#","")).appendChild(s)}catch(e){location.reload()}} });
